  ...
  <Row className="g-3" aria-hidden="true">
    {[...Array(6)].map((_, i) => (
      <Col key={i} xs={12} md={6} lg={4} xl={3}>
        <Card className="h-100 recent-card" style={{ cursor: 'default' }}>
          <Card.Body className="d-flex align-items-center">
            ...
            <div className="flex-grow-1" style={{ marginLeft: 12 }}>
              <div style={{
              ...
              <div style={{ height: 8 }} />
              <div style={{
                height: 12, width: '90%', borderRadius: 6,
                background: 'linear-gradient(90deg, #f1f5f9 25%, #e5e7eb 37%, #f1f5f9 63%)',
                backgroundSize: '400% 100%',
                animation: 'shimmer 1.4s ease infinite'
              }} />
...
>
  ...
  const UsersTab = () => {
    ...
    const filtered = searched.filter(u => {
      const byRole = roleFilter ? String(u.role || '').toLowerCase() === roleFilter.toLowerCase() : true;
      const byDept = deptFilter ? String(u.department_name || '').toLowerCase() === deptFilter.toLowerCase() : true;
    ...
    const SortIcon = ({ col }) => (
      <span style={{ marginLeft: 6, opacity: sortKey === col ? 1 : 0.3 }}>
      ...
      <div style={styles.tabContent} className="dashboard-tab-content">
        ...
        <div className="d-flex flex-wrap" style={{ gap: 8, marginBottom: 12 }}>
          ...
          <div className="ms-auto d-flex align-items-center" style={{ gap: 6 }}>
            <span className="text-muted" style={{ fontSize: 12 }}>Rows:</span>
            <select
              value={pageSize}
        ...
        {loading ? (
          ...
          {/* Pagination row matching trash design */}
          <div className="mt-3">
            <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center" style={{ gap: '8px' }}>
              <div className="text-muted mb-2 mb-md-0" style={{ fontSize: 12 }}>
                Showing {total === 0 ? 0 : start + 1}-{Math.min(start + pageSize, total)} of {total} users
                <span className="ms-2">({total} total)</span>
              </div>
              <div className="d-flex flex-wrap" style={{ gap: '6px' }}>
                <button
                  className="btn btn-light border"
                  style={{ borderRadius: 6, padding: '4px 7px', fontSize: '80%' }}
                  disabled={safePage === 1}
                  onClick={() => setPage(1)}
                  title="First Page"
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <button
                      key={pageNum}
                      className={`btn ${safePage === pageNum ? 'btn-primary' : 'btn-light'} border`}
                      style={{ borderRadius: 6, padding: '3.5px 6px', minWidth: 24, fontSize: '75%' }}
         ...
         <div style={styles.modalOverlay}>
           <div style={styles.modal}>
             ...
             <div style={styles.modalFooter}>
               ...
               <button 
                 onClick={() => {
                   if (selectedDocument.file_url) {
                     ...
                     // Close modal and switch to documents tab
                     setShowDocumentModal(false);
                     setSelectedDocument(null);
               ...
               >
                 {selectedDocument.file_url ? 'Download Document' : 'View in Documents'}
               </button>
             </div>
           </div>
         </div>
