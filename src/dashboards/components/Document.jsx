...
const Document = ({ role, onOpenTrash, onNavigateToUpload, onNavigateToUpdate }) => {
  ...
  const visibleFolders = useMemo(() => {
    ...
    // For others, only show folders that contain at least one visible document
    return withCounts.filter(f => f.count > 0);
  }, [filteredDocuments, folders, effectiveIsDean, isAdmin]);
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {!isUser && (
              <>
                ...
                <button 
                  onClick={() => (onOpenTrash ? onOpenTrash() : setShowTrashcan(true))} 
                  className="btn btn-light border rounded-pill"
                  style={{
                    flex: '1',
                    padding: '8px 12px',
            ...
            <span style={{ marginLeft: isMobile ? 0 : 'auto', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', width: isMobile ? '100%' : 'auto' }}>
              ...
              <input
                ...
                style={{
                  ...
                  fontSize: '12px',
                  background: '#fff',
                  flex: isMobile ? '1 1 120px' : '0 0 200px',
                  minWidth: 0,
                  boxSizing: 'border-box',
                  maxWidth: '100%'
                }}
              />
              <button className="btn btn-light btn-sm border rounded-circle" title="Scroll left" aria-label="Scroll folders left" onClick={() => scrollFolders('left')}>
                «
              </button>
              <button className="btn btn-light btn-sm border rounded-circle" title="Scroll right" aria-label="Scroll folders right" onClick={() => scrollFolders('right')}>
                »
              </button>
            </span>
          </h3>
            ...
            <div key={index} className="card shadow-sm">
              ...
              <div className="card-body">
                <div className="d-flex align-items-start gap-3">
                  {doc.created_by_profile_pic ? (
                    <img 
                      src={doc.created_by_profile_pic} 
                      alt="avatar" 
                      style={{ 
                        width: '48px', 
                        height: '48px', 
                    ...
                    >
                      {(doc.created_by_name || '?').charAt(0).toUpperCase()}
                    </div>
                  )}
                  <div className="flex-grow-1">
                    ...
                    <div className="text-muted small">
                      <div className="mb-1">
                        <span style={{ fontWeight: '500' }}>Created by:</span> {doc.created_by_name || 'Unknown'}
                      </div>
                      <div>
                  ...
                  >
                    <div className="d-grid gap-2">
                      ...
                      <button 
                        ...
                        style={{ border: 'none', padding: '10px 12px', fontSize: '14px' }}
                      >
                        <InfoCircle size={14} style={{ marginRight: 8 }} />
                        Properties
                      </button>
                      {hasAdminPrivileges(role) && (
                        <>
                          ...
                          <button 
                            className="btn btn-sm btn-light text-start text-danger"
                            onClick={(e) => { 
                              e.preventDefault();
                              e.stopPropagation(); 
                              setOpenDropdown(null);
