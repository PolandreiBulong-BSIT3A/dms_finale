...
const UserTrash = ({ onBack, currentUser, isAdmin, effectiveIsDean }) => {
  ...
  const handleSelectAll = (checked) => {
    // Select only the users on the current page to match checkbox state and UX
    if (checked) setSelectedIds(paginatedUsers.map(u => u.id));
    else setSelectedIds([]);
  };
  ...
  return (
    <div style={styles.container}>
          ...
          {isMobile ? (
            ...
            <>
              <div className="d-flex align-items-center justify-content-between" style={{ marginBottom: '8px' }}>
                <label className="d-flex align-items-center" style={{ gap: '8px', margin: 0 }}>
                  <input
                    type="checkbox"
                    ...
                    <div className="d-flex align-items-start gap-3">
                      ...
                      <div className="flex-grow-1">
                        <div className="d-flex justify-content-between align-items-start">
                          <div>
                            <h6 className="mb-0 fw-bold" style={{ fontSize: '1rem', color: '#111827' }}>{user.name}</h6>
                        ...
                        <div className="text-muted small mt-2 d-flex align-items-center gap-2">
                          <Envelope size={14} color="#9ca3af" />
                          <span style={{ wordBreak: 'break-word' }}>{user.email || 'No email'}</span>
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      {filtered.length > 0 && (
        <div className={isMobile ? 'mt-2 d-flex flex-column align-items-center text-center' : 'd-flex align-items-center justify-content-between mt-2'} style={{ gap: isMobile ? 8 : 12 }}>
          <div className="text-muted" style={{ fontSize: 12 }}>
            Showing {filtered.length === 0 ? 0 : startIndex + 1}-{endIndex} of {filtered.length} users
            <div style={{ fontSize: 12 }}>(
            {filtered.length} total)</div>
          </div>
          <div className={isMobile ? 'd-flex align-items-center justify-content-center' : 'd-flex align-items-center'} style={{ gap: 6 }}>
            <button
            ...
            {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
              ...
              return (
                <button
                  key={pageNum}
                  className={`btn ${currentPage === pageNum ? 'btn-primary' : 'btn-light'} border`}
                  style={{ borderRadius: 10, padding: '6px 10px', minWidth: 36 }}
