...
router.put('/users/update-profile', requireAuth, async (req, res) => {
  try {
    ...
    // Get old username before updating if username is being changed
    if (username !== undefined) {
      const [userResults] = await db.promise().query(
        'SELECT Username FROM dms_user WHERE user_email = ?',
        [email]
      );
      if (userResults.length > 0) {
        oldUsername = userResults[0].Username;
      }
    ...
    // If username was changed, update usernames in all related tables
    if (username !== undefined && oldUsername && oldUsername !== username) {
      try {
        // Get all possible variations of the old username to update
        const [variations] = await db.promise().query(
          'SELECT DISTINCT created_by_name FROM dms_documents WHERE created_by_name LIKE ? OR created_by_name = ?',
          [`%${oldUsername}%`, oldUsername]
        );
        
        const usernameVariations = [oldUsername];
        variations.forEach(row => {
          if (row.created_by_name && !usernameVariations.includes(row.created_by_name)) {
            usernameVariations.push(row.created_by_name);
          }
        });

        console.log(`Found ${usernameVariations.length} username variations to update:`, usernameVariations);

        // Update each variation
        for (const variation of usernameVariations) {
          // Update documents
          await db.promise().query(
            'UPDATE dms_documents SET created_by_name = ? WHERE created_by_name = ?',
            [username, variation]
          );

          // Update trashcan (deleted_by_name)
          await db.promise().query(
            'UPDATE dms_documents SET deleted_by_name = ? WHERE deleted_by_name = ?',
            [username, variation]
          );

          // Update announcements
          await db.promise().query(
            'UPDATE announcements SET created_by_name = ? WHERE created_by_name = ?',
            [username, variation]
          );
          ...
          try {
            await db.promise().query(
              'UPDATE notifications SET created_by = ? WHERE created_by = ?',
              [username, variation]
            );
          } catch (notifError) {
            console.warn('Notifications table update failed:', notifError.message);
          }

          // Update requests (if exists)
          try {
            await db.promise().query(
              'UPDATE requests SET created_by_name = ? WHERE created_by_name = ?',
              [username, variation]
            );
          } catch (reqError) {
            console.warn('Requests table may not exist or have different structure:', reqError.message);
          }

          // Update document_actions (if exists)
          try {
            await db.promise().query(
              'UPDATE document_actions SET created_by_name = ? WHERE created_by_name = ?',
              [username, variation]
            );
          } catch (actionError) {
            console.warn('Document actions table may not exist or have different structure:', actionError.message);
          }

          console.log(`Updated all tables from "${variation}" to "${username}"`);
        }

      } catch (updateError) {
        console.warn('Failed to update usernames in some tables:', updateError);
        // Don't fail the profile update if some updates fail
      }
    }
