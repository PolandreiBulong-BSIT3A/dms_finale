...
const initializeDocumentTypesSchema = async () => {
  try {
    ...
    try {
      const [idxRows] = await db.promise().execute(
        `SELECT 1 FROM information_schema.statistics
         WHERE table_schema = DATABASE()
           AND table_name = 'document_types'
           AND index_name = 'uniq_document_types_name'`
      );
      if (idxRows.length === 0) {
        await db.promise().execute('CREATE UNIQUE INDEX uniq_document_types_name ON document_types (name)');
        console.log('Created unique index on document_types(name)');
      }
    } catch (e) {
      console.warn('Unique index ensure warning (document_types):', e?.message || e);
    }
  } catch (error) {
