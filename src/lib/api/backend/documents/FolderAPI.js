...
const ensureFolderIndexes = async () => {
  try {
    // Unique index on folders(name)
    const [idxRows1] = await db.promise().execute(
      `SELECT 1 FROM information_schema.statistics
       WHERE table_schema = DATABASE()
         AND table_name = 'folders'
         AND index_name = 'uniq_folders_name'`
    );
    if (idxRows1.length === 0) {
      // Attempt to create unique index (case-insensitive depends on table collation)
      await db.promise().execute(`CREATE UNIQUE INDEX uniq_folders_name ON folders (name)`);
    }

    // Composite unique on document_folders(doc_id, folder_id)
    const [idxRows2] = await db.promise().execute(
      `SELECT 1 FROM information_schema.statistics
       WHERE table_schema = DATABASE()
         AND table_name = 'document_folders'
         AND index_name = 'uniq_doc_folder_pair'`
    );
    if (idxRows2.length === 0) {
      await db.promise().execute(`CREATE UNIQUE INDEX uniq_doc_folder_pair ON document_folders (doc_id, folder_id)`);
    }
  } catch (e) {
    console.warn('ensureFolderIndexes warning:', e?.message || e);
  }
};

// Kick off index checks (non-blocking)
ensureFolderIndexes();
